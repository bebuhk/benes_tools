...
def plot_external_potential_3D(grid_xyz, external_potential=None, lattice=None,
    ...
    - external_pot_grid: 3D NumPy array containing the external potential values [K] at each grid point (shape: (nx, ny, nz))
    ...
    title="External Potential V<sup>ext</sup> [K]"
    show_colorbar = True
    if external_potential is None:
        external_potential = np.ones(grid_xyz.shape[:-1])*np.nan
    ...
    potentials = external_potential.flatten()
    ...
    scatter_plot = go.Scatter3d(
        ...
        text=[f"V<sup>ext</sup>: {p:.2f} K" for p in potentials], # Add potential values to hover text
